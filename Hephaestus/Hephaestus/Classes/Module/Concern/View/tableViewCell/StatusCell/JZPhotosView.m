...
- (void)setUpAllChildView
{
    for (int i = 0; i < 9; i++) {
        ...
        JZPhotoView *imageV = [[JZPhotoView alloc] init];
...
- (void)tap:(UITapGestureRecognizer *)tap
{
    ...
    for (JZPhoto *photo in _pic_urls) {
...
- (void)setPic_urls:(NSArray *)pic_urls
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        JZPhotoView *imageV = self.subviews[i];
        
        if (i < _pic_urls.count) { // 显示
            imageV.hidden = NO;
            
            // 获取CZPhoto模型
            JZPhoto *photo = _pic_urls[i];
